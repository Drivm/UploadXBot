    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                "Upload limit of Telegram: 1.9GB\n"
                f"<b>File size:</b> {d_f_s}\n"
                "\nSplitting the file..."
            )
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    if os.path.isfile(file_upload):
        ...
        await messa_ge.reply_text(
            f"<b>Uploaded:</b>`{os.path.basename(file_upload)}` <a href='tg://user?id={g_id}'></a>\nSize: {gjay}",
...
async def upload_single_file(
    ...
    # todo
        ...
        sent_message = await message.reply_document(
            ...
            progress_args=(
                f"<b>File:</b> `{os.path.basename(local_file_name)}`",
                start_time,
            ),
        )
            ...
            if local_file_name.upper().endswith(("MKV", "MP4", "WEBM", "FLV", "3GP", "AVI", "MOV", "OGG", "WMV", "M4V", "TS", "MPG", "MTS", "M2TS")):
                ...
                if edit_media and message.photo:
                    ...
                    sent_message = await message.reply_audio(
                        ...
                        progress=prog.progress_for_pyrogram,
                        progress_args=(
                            f"Uploading...\n\n<b> File</b>: `{os.path.basename(local_file_name)}`",
                            start_time,
                        ),
                    )
                if thumb is not None:
