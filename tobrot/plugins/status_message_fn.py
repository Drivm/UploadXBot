...
async def upload_as_doc(client, message):
    ...
    await message.reply_text(f"File will be uploaded as <code>document</code> for {u_men}")


async def upload_as_video(client, message):
    user_specific_config[message.from_user.id]=UserDynaConfig(message.from_user.id,False)
    u_men = message.from_user.mention
    await message.reply_text(f"File will be  uploaded as <code>streamable media</code> for ? {u_men}")
    ...
    while True:
        ...
        for file in downloads:
            ...
            if file.status == "active":
                ...
                msg += f"\n{msgg}"
                msg += f"\n<b>Cancel</b>: <code>/{CANCEL_COMMAND_G} {file.gid}</code>"
        ...
        ms_g = (
            ...
            f"<b>FREE:</b> <code>{free}</code>  <b>UP</b>:<code>{hr}h{mi}m{se}s</code>\n"
...
async def cancel_message_f(client, message):
    if len(message.command) > 1:
        ...
        try:
            ...
            await i_m_s_e_g.edit_text("" + str(e) + "")
    else:
        await message.delete()
...
async def upload_log_file(client, message):
    ...
    if g:
        await message.reply_document("bot-logs.txt")
